...
class VideoFrameExtractor: ObservableObject {
    ...
    private func createImageGenerator(for asset: AVAsset, maximumSize: CGSize = CGSize(width: 1920, height: 1080)) -> AVAssetImageGenerator {
    ...
    func extractFrames(from asset: AVAsset, 
        ...
        let generator = createImageGenerator(for: asset)
    ...
    func generateThumbnail(from asset: AVAsset, at time: CMTime? = nil, size: CGSize = CGSize(width: 300, height: 300)) async throws -> UIImage {
        ...
        let generator = createImageGenerator(for: asset, maximumSize: size)
        
        // Use iOS 15 compatible method
        return try await withCheckedThrowingContinuation { continuation in
            generator.generateCGImagesAsynchronously(forTimes: [NSValue(time: targetTime)]) { _, image, _, result, error in
                if let error = error {
                    ...
                    continuation.resume(throwing: NSError(domain: "VideoFrameExtractor", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to generate image"]))
                }
            }
        }
    }

...
class VideoExporter: ObservableObject {
    ...
    func exportVideo(project: VideoProject, 
        ...
        print("🎞️ Total frames in project: \(project.totalFrames)")
        print("📊 Selected frames: \(project.selectedFrames.count)")
        
        guard let originalAsset = project.asset else {
            print("❌ No original asset found")
            throw ExportError.noOriginalAsset
        }
        
        guard !project.selectedFrames.isEmpty else {
            print("❌ No frames selected for export")
            throw ExportError.missingFrameData
        }
        
        print("✅ Original asset duration: \(originalAsset.duration.seconds) seconds")
    ...
    private func setupAssetWriter(outputURL: URL, settings: ExportSettings) async throws {
        ...
        print("📊 Video bitrate: \(settings.videoBitrate)")
        print("🎥 Video codec: \(settings.videoCodec)")
    ...
    private func processAndWriteFrames(project: VideoProject, 
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            func processNextBatch() {
                while assetWriterInput.isReadyForMoreMediaData && currentFrameIndex < totalFrames {
                    let frame = selectedFrames[currentFrameIndex]
                    let presentationTime = CMTime(value: Int64(currentFrameIndex), timescale: Int32(project.frameRate))
                    
                    do {
                        let pixelBuffer = try self.createPixelBuffer(from: frame, project: project, settings: settings)
                        ...
                        DispatchQueue.main.async {
                            self.exportProgress = Double(currentFrameIndex) / Double(totalFrames) * 0.8
                            self.exportStatus = "Processing frame \(currentFrameIndex) of \(totalFrames)"
                        }
    ...
    private func createPixelBuffer(from frame: FrameAdjustment, project: VideoProject, settings: ExportSettings) throws -> CVPixelBuffer {
        guard let thumbnail = frame.thumbnail else {
            print("❌ Missing thumbnail for frame \(frame.frameIndex)")
            throw ExportError.missingFrameData
        }
        
        print("🖼️ Processing frame \(frame.frameIndex) with thumbnail size: \(thumbnail.size)")
        
        
        let ciImage = CIImage(image: thumbnail) ?? CIImage()
        
        // Use frame-specific adjustments if frame is marked, otherwise use global adjustments
        let adjustmentsToApply: AdjustmentSet
        if project.markedFrameIndices.contains(frame.frameIndex) {
            // Frame is marked - use its specific adjustments
            adjustmentsToApply = frame.adjustments
        } else {
            // Frame not marked - use global adjustments
            adjustmentsToApply = project.globalAdjustments
        }
        
        let processedImage = imageProcessor.processImageSync(ciImage, adjustments: adjustmentsToApply)
...
struct ExportSettings {
    ...
    static let presets: [ExportPreset] = [
        // PROFESSIONAL CINEMA EXPORT FORMATS
        ExportPreset(
            name: "ProRes 4444 - Cinema Master",
            settings: ExportSettings(
                resolution: CGSize(width: 4096, height: 2160), // Cinema 4K
                frameRate: 24.0, // Cinema standard
                videoBitrate: 330_000_000, // ProRes 4444 bitrate
                videoCodec: .hevc, // Use HEVC for high quality instead of ProRes
                fileType: .mov,
                profileLevel: AVVideoProfileLevelH264HighAutoLevel,
                includeAudio: true,
                audioSampleRate: 48000,
                audioChannels: 2,
                audioBitrate: 1536_000, // Uncompressed audio
                transform: .identity
            )
        ),
        ExportPreset(
            name: "4K60 - Ultra High Quality",
            settings: ExportSettings(
                resolution: CGSize(width: 3840, height: 2160),
                frameRate: 60.0, // 60fps for smooth motion
                videoBitrate: 80_000_000, // Ultra high bitrate
                videoCodec: .hevc,
                fileType: .mov,
                profileLevel: AVVideoProfileLevelH264HighAutoLevel,
                includeAudio: true,
                audioSampleRate: 96000, // Studio quality audio
                audioChannels: 2,
